...
def load_data():
    ...
    df = pd.read_csv('time_series_covid19_confirmed_global.csv',parse_dates=['Date'])
...
def tts(data):
    ...
    months of data.
    """
    data = data.drop(['Confirmed', 'Date'], axis=1)
...
def predict_df(unscaled_predictions, original_df):
    ...
    result_list = []
    confirmed_dates = list(original_df[-13:].Date)
    act_confirmed = list(original_df[-13:].confirmed)

    for index in range(0, len(unscaled_predictions)):
        result_dict = {}
        result_dict['pred_value'] = int(unscaled_predictions[index][0] +
                                        act_sales[index])
        result_dict['Date'] = confirmed_dates[index+1]
